...
- **마지막 문서 업데이트**: 2026-02-13 (43차)

---

## 2026-02-13 (43차) 이석 잠금화면·PC-ON 비밀번호 모달·보조 모니터 안정화

**작업 요약**

| 구분 | 내용 |
|------|------|
| **이석 시 잠금화면 표시** | 이석 감지 시 주 창에도 **이석(empty)** 데이터 전달: showLockInWindow/createLockWindow에서 `lastWorkTimeData.screenType = "empty"`, `leaveSeatOffInputMath` 강제. 주 창·createLockWindow 새 창에 **did-finish-load**로 `pcoff:lock-initial-work` 전송. config의 `leaveSeatUnlockRequirePassword`·`lockScreen.leave`(설정된 이석 문구·배경) 병합. |
| **설정된 이석 잠금화면** | **applyLockScreenLeaveFromConfig()** 추가 — 이석(screenType=empty)일 때 config.lockScreen.leave(title, message, backgroundUrl, logoUrl) 병합. showLockInWindow/createLockWindow·getWorkTime(모든 반환 경로)에서 호출. |
| **PC-ON 비밀번호 모달** | lock.js: **currentWork/currentLeaveSeatPolicy** 공유, onLockInitialWork 수신 시 갱신. PC-ON 클릭 시 **getWorkTime() 재조회**로 이석·비밀번호 여부 판단. 이석이면 `leaveSeatUnlockRequirePassword !== false` 시 비밀번호 모달 표시(설정 미지정 시 기본 표시). coerceWorkTimeFromApi에서 이석일 때 leaveSeatUnlockRequirePassword 기본 true. getWorkTime 이석 시 screenType=empty 고정·applyLeaveSeatUnlockRequirePasswordFromConfig 적용. |
| **버그 수정** | bootstrap 로그 블록에서 **leaveSeatPolicy → currentLeaveSeatPolicy** (ReferenceError 해소). |
| **보조 모니터 잠금** | 보조에 잠금화면이 간혹 안 뜨던 현상: **ensureLockWindowsForAllDisplays()**를 createLockWindow/showLockInWindow에서 **await** 후, **500ms 뒤 재호출**해 디스플레이 목록 지연 시 보조 창 생성 보완. |

**파일 변경**

- `app/main/index.ts`: applyLockScreenLeaveFromConfig(), 이석 시 lastWorkTimeData 강제·config 병합, getWorkTime 이석 분기·leave 문구/비밀번호 보장, ensureLockWindowsForAllDisplays await + 500ms 재시도, 주 창/새 창 did-finish-load 전송
- `app/renderer/lock.js`: currentWork/currentLeaveSeatPolicy, onLockInitialWork 갱신, PC-ON 시 getWorkTime 재조회, 이석 비밀번호 모달 조건(needLeaveSeatPassword), leaveSeatPolicy→currentLeaveSeatPolicy
- `config.example.json`: leaveSeatUnlockRequirePassword: true 추가
- `docs/개발_이력_리포트.md`: 43차 추가

---

## 2026-02-13 (42차) 긴급해제 PHP 연동·404 안내·이석 체크 정책 조정

**작업 요약**

| 구분 | 내용 |
|------|------|
| **긴급해제 PHP 검증** | 원본(5240.PcOff WebView)과 동일하게 **PHP(sendLockPass.php)** 우선 사용. `config.sendLockPassUrl` 또는 `lockScreenApiUrl` 도메인 기반 `/Includes/sendLockPass.php`로 POST(ServAreaID, LockPass), 응답 `result === 'success'`면 성공. 서버에 API 없을 때 대응. |
| **404 사용자 안내** | `callPcOffEmergencyUnlock.do` 404(Not Found) 시 메시지를 "서버에 긴급해제 API가 등록되지 않았습니다. 관리자에게 문의하세요."로 변경. |
| **이석 체크 로직** | **임시연장(TEMP_EXTEND)·긴급사용(EMERGENCY_USE)**: 이석 체크 **적용**(유휴/절전 시 이석 잠금 표시). **긴급해제(EMERGENCY_RELEASE)**: 이석 체크 **제외**. `showLockForLocalLeaveSeat()`에서 EMERGENCY_RELEASE일 때만 return. |
| **문서 정책 반영** | PRD FR-11, TRD §9.5, docs/다음_개발_진행_사항.md에 "긴급해제 중에만 이석 미표시, 임시연장·긴급사용은 이석 체크 적용" 명시. |

**파일 변경**

- `app/core/emergency-unlock.ts`: attemptViaPhp(), 404 시 안내 문구
- `app/main/index.ts`: getSendLockPassUrl(), pcoff:requestEmergencyUnlock에서 PHP 우선 분기, showLockForLocalLeaveSeat() 이석 제외 조건을 EMERGENCY_RELEASE만으로 축소
- `config.example.json`: sendLockPassUrl 예시
- `PRD_5240_PcOff_Electron.md`, `TRD_5240_PcOff_Electron.md`, `docs/다음_개발_진행_사항.md`: 이석 적용 조건 문구 수정
- `docs/원본_긴급해제_설정_확인.md`: §4 PHP 우선 사용 기준
- `docs/개발_이력_리포트.md`: 42차 추가

---

## 2026-02-13 (41차) 보조 모니터 — 임시연장 카운트 동기화·긴급해제 시 디스플레이 배치

**작업 요약**

| 구분 | 내용 |
|------|------|
| **보조 모니터 임시연장 카운트 0** | 보조 잠금창에 임시연장 배지가 0으로만 나오던 문제. **원인**: onLockInitialWork 수신 시 extendCountEl 미갱신, 재사용 보조창에 lock-initial-work 미전송, 근태 갱신 시 보조창 미동기화. |
| **수정(임시연장)** | **lock.js**: onLockInitialWork 콜백에서 `extendCountEl.textContent = String(work.pcExCount ?? 0)` 추가. **main**: ensureLockWindowsForAllDisplays()에서 기존 보조창 재사용 시에도 `pcoff:lock-initial-work` 전송; refreshWorkTimeFromApi() 후 `broadcastWorkTimeToLockWindows()` 호출로 주·보조 모든 잠금창에 lastWorkTimeData 동기화. |
| **긴급해제 시 반응** | 보조 모니터에서 긴급해제 성공 후 작동정보 창이 주 모니터에만 떠 사용자가 반응을 못 느끼던 문제. **수정**: requestEmergencyUnlock 성공 시 event.sender 창의 디스플레이를 구해 `showTrayInfoInCurrentWindow(onDisplay)` 로 전달; 해당 디스플레이 workArea 중앙에 작동정보 창 배치·포커스. |

**미해결(다음 점검)**

- **긴급사용 모달**: 보조 모니터에서 긴급사용 버튼 클릭 시 모달이 뜨지 않음.
- **긴급해제 버튼**: 주·보조 모니터 모두에서 긴급해제 버튼 클릭 시 동작하지 않음.

**파일 변경**

- `app/renderer/lock.js`: onLockInitialWork 시 extendCountEl 갱신
- `app/main/index.ts`: broadcastWorkTimeToLockWindows(), ensureLockWindowsForAllDisplays 시 기존 보조창에 lock-initial-work 전송, refreshWorkTimeFromApi 후 broadcast, showTrayInfoInCurrentWindow(onDisplay?), requestEmergencyUnlock에서 sender 디스플레이 전달
- `docs/보조모니터_이슈.md`: §6 보완 작업·§7 미해결 버튼 동작 정리
- `docs/개발_이력_리포트.md`: 41차 추가

---

## 2026-02-13 (40차) 추후 개발·점검 1→2→3 연동·E2E 준비

**작업 요약**

| 구분 | 내용 |
|------|------|
| **통합 실행 스크립트** | `scripts/run-integration-check-1-2-3.mjs` — 서버 구현 후 시뮬레이터 run-all·잠금화면 설정 검증·체크리스트 출력·(선택) 임시연장 로그 검증을 한 번에 실행. 옵션: `--logs=./logs`, `--skip-simulator`. |
| **잠금화면 Fallback 검증** | `scripts/check-lock-screen-config.mjs` — config.json의 lockScreen·lockScreenApiUrl 준비 상태 검사(2번 Fallback 가능 여부). |
| **연동 가이드** | `docs/추후_개발_점검_연동_가이드.md`에 **서버 구현 완료 조건** 표 추가(1→2→3별 서버 측 완료 조건). 통합 실행·설정 검증 스크립트 안내 및 §2.3 체크리스트에 설정 검증 항목 추가. |
| **다음_개발_진행_사항** | 우선 마무리 순서 문단에 통합 실행 명령 추가. 1·2·3 항목 테이블에 에이전트 검증 스크립트( run-integration-check-1-2-3.mjs, check-lock-screen-config.mjs, check-temp-extend-reflect.mjs ) 참조 추가. |

**파일 변경**

- `scripts/run-integration-check-1-2-3.mjs`: 신규
- `scripts/check-lock-screen-config.mjs`: 신규
- `docs/추후_개발_점검_연동_가이드.md`: 서버 구현 완료 조건·통합 실행·설정 검증 안내
- `docs/다음_개발_진행_사항.md`: 통합 실행·스크립트 참조
- `docs/개발_이력_리포트.md`: 40차 추가

---

## 2026-02-13 (39차) 임시연장 잠금화면 배지·최대 횟수 초과 대응

**작업 요약**

| 구분 | 내용 |
|------|------|
| **잠금화면 배지 0 표시** | 잠금창 띄우기 직전 선로드(createLockWindow/showLockInWindow)·주기 잠금 검사(isLockRequired)·getWorkTime 등에서 API 응답으로 lastWorkTimeData를 덮어쓸 때 서버가 pcExCount 0을 주면 로컬에 있던 사용 횟수가 사라져 배지가 0으로 보이던 문제. |
| **수정** | **preserveLocalPcExCountIfHigher(apiData)** 공통 함수 추가 — 서버 pcExCount가 로컬보다 작으면 같은 객체에 pcExCount·pcOffYmdTime만 로컬 값으로 보정. API 응답으로 lastWorkTimeData를 갱신하는 **모든 경로**에 적용: createLockWindow, showLockInWindow, isLockRequired(EMERGENCY_USE·일반 fetch), getWorkTime, refreshWorkTimeFromApi, PC-ON/이석해제 직후 갱신, refreshMyAttendance. |
| **임시연장 최대 횟수 초과** | API 문서상 pcExCount < pcExMaxCount일 때만 callPcOffTempDelay 호출. **Main**: requestPcExtend에서 currentCount >= maxCount이면 API 호출 없이 "임시연장 가능 횟수를 모두 사용했습니다." 반환. **잠금화면·작동정보**: applyButtonDisp에서 pcExCount >= pcExMaxCount이면 임시연장 버튼 숨김(hasQuota). |

**파일 변경**

- `app/main/index.ts`: preserveLocalPcExCountIfHigher() 추가 및 8개 경로에 적용, requestPcExtend에서 최대 횟수 초과 시 차단
- `app/renderer/lock.js`: applyButtonDisp 종업(off) 시 hasQuota(pcExCount < pcExMaxCount) 조건으로 임시연장 버튼 표시
- `app/renderer/renderer.js`: applyButtonDisp 동일 적용
- `docs/개발_이력_리포트.md`: 39차 추가
- `docs/다음_개발_진행_사항.md`: 버튼 노출 기준(임시연장 소진 시 숨김)·잠금화면 배지(전 경로 preserve) 문구 반영

---

## 2026-02-27 (38차) Windows 다중 모니터 — 잠금 해제 시 보조 모니터 잠금 화면 미사라짐 수정

**작업 요약**

| 구분 | 내용 |
|------|------|
| **원인** | 잠금 해제 후 주 모니터는 에이전트 화면으로 전환되는데, Windows에서 보조 모니터의 잠금창을 `destroy()`만 할 경우 풀스크린 해제가 늦어 잠금 화면이 남는 현상. |
| **수정** | `closeAllLockWindows()`에서 보조 잠금창마다 **setFullScreen(false) → hide() → destroy()** 순으로 처리. 풀스크린 해제 후 즉시 숨기면 보조 모니터에서도 잠금이 사라짐. |

**파일 변경**

- `app/main/index.ts`: closeAllLockWindows() — 보조창 닫기 전 setFullScreen(false)·hide() 후 destroy()
- `docs/다음_개발_진행_사항.md`: 다중 디스플레이 문단에 closeAllLockWindows 동작·Windows 대응 설명 반영
- `docs/다중_디스플레이_QA.md`: 해제 시 보조창 닫기 순서(풀스크린 해제→hide→destroy) 명시
- `docs/개발_이력_리포트.md`: 38차 추가

---

## 2026-02-27 (37차) 추후 개발·점검 — 연동 가이드·leave_seat_unlock 시나리오

**작업 요약**

| 구분 | 내용 |
|------|------|
| **연동·E2E 가이드** | `docs/추후_개발_점검_연동_가이드.md` 신규 — 추후 개발·점검 1~3(이석 해제 비밀번호, 잠금화면 문구 API, 임시연장 서버 반영)에 대한 전제 조건·설정 예시·E2E 시나리오·연동 확인 절차·Fallback 로그·시뮬레이터 안내. |
| **시뮬레이터** | `leave_seat_unlock` 시나리오 추가 — config.json 읽기, mock/로컬/URL/.do 우선순위 검증, UNLOCK_TRIGGERED 로그 기록. Flow-02c, FR-11·FR-14. `simulator/scenarios.ts`, `simulator/cli.ts`(ALL_SCENARIOS) 반영. |
| **다음_개발_진행_사항** | 이석 해제 문단에 에이전트 측 추가(시나리오·가이드 §2 fallback 로그) 반영. 우선 마무리 순서(추후 개발·점검 1→2→3) 및 가이드 링크 정리. |

**파일 변경**

- `docs/추후_개발_점검_연동_가이드.md`: 신규
- `simulator/scenarios.ts`: ScenarioName·runLeaveSeatUnlockScenario 추가
- `simulator/cli.ts`: ALL_SCENARIOS에 leave_seat_unlock 추가
- `docs/다음_개발_진행_사항.md`: 추후 개발 우선 마무리·가이드 링크·이석 해제 에이전트 추가
- `docs/개발_이력_리포트.md`: 37차 추가

---

## 2026-02-13 (36차) SmartScreen·업데이트 무결성 — 실패 구분 로깅·체크리스트

**작업 요약**

| 구분 | 내용 |
|------|------|
| **로그 코드** | `app/core/constants.ts`에 업데이트 실패 구분용 3종 추가: `UPDATE_SIGNATURE_INVALID`, `UPDATE_HASH_MISMATCH`, `UPDATE_NETWORK_ERROR`. (기존 `UPDATE_FAILED`는 미분류 실패용 유지.) |
| **UpdateManager** | `app/core/update-manager.ts`에 `getUpdateFailureLogCode(message)` 도입 — 오류 메시지 패턴( signature / hash·checksum·integrity / network·econnrefused·enotfound 등)에 따라 위 3종 또는 `UPDATE_FAILED` 반환. `autoUpdater.on("error")` 및 `enqueueRetry` 로깅 시 해당 코드 사용. |
| **체크리스트 문서** | `docs/smart-screen-checklist.md` 신규 — 코드 서명(Authenticode·notarization), SmartScreen 평판, 업데이트 무결성(구분 로그 참고), 설치기 권한, 오탐 대응, 릴리즈 전·중·후 항목 정리. |
| **문서 연계** | `docs/업데이트_릴리즈_점검.md`: 업데이트 로그 검색 안내에 구분 코드 및 smart-screen-checklist.md 링크 추가. `docs/다음_개발_진행_사항.md` §12: 저장소 적용 중 구분 로깅·체크리스트 완료 문구 반영. |

**파일 변경**

- `app/core/constants.ts`: UPDATE_SIGNATURE_INVALID, UPDATE_HASH_MISMATCH, UPDATE_NETWORK_ERROR 추가
- `app/core/update-manager.ts`: getUpdateFailureLogCode(), error·enqueueRetry에서 해당 코드 사용
- `docs/smart-screen-checklist.md`: 신규
- `docs/업데이트_릴리즈_점검.md`: 구분 로그·체크리스트 링크
- `docs/다음_개발_진행_사항.md`: §12 저장소 적용 완료 항목
- `docs/개발_이력_리포트.md`: 36차 추가

---

## 2026-02-13 (35차) Kill 통제 — 연동 레이어(감사 로그·quit)

**작업 요약**

| 구분 | 내용 |
|------|------|
| **연동 레이어** | `app/core/kill-control.ts` — `requestKillWithLog`(요청 등록 + KILL_REQUEST_CREATED), `verifyKillOtpWithLog`(OTP 검증 + KILL_OTP_VERIFIED/KILL_TOKEN_ISSUED 또는 KILL_OTP_FAILED), `executeKillWithToken`(fingerprint·killExecute·성공 시 KILL_EXECUTED 로그 후 appQuit(), 실패 시 KILL_REJECTED). |
| **체크리스트** | 5.1 #4·#5 완료 처리. 에이전트 담당 항목 전부 ✅. |

**파일 변경**

- `app/core/kill-control.ts`: 신규
- `docs/kill-control-phase1.md`: 5.1 #4·#5 ✅
- `docs/다음_개발_진행_사항.md`: Phase 1 에이전트 완료 문구 정리
- `docs/개발_이력_리포트.md`: 35차 추가

---

## 2026-02-13 (34차) Kill 통제 — Process Fingerprint 수집 유틸

**작업 요약**

| 구분 | 내용 |
|------|------|
| **유틸** | `app/core/kill-fingerprint.ts` — `getProcessFingerprint()`: process.pid, process.execPath, process.argv → cmdLine, exePath 파일 SHA-256 해시(최대 10MB). kill-execute payload용. |
| **문서** | `docs/kill-control-phase1.md` 체크리스트 5.1 #3 완료 처리, 감사 로그·quit 연동 시 삽입 위치 안내 추가. |

**파일 변경**

- `app/core/kill-fingerprint.ts`: 신규
- `docs/kill-control-phase1.md`: 체크리스트·연동 안내

---

## 2026-02-13 (33차) Kill 통제 Phase 1 — 로그 코드·설계 문서·API 스텁

**작업 요약**

| 구분 | 내용 |
|------|------|
| **로그 코드** | FR-18 Kill 통제용 8종 상수 추가: `KILL_REQUEST_CREATED`, `KILL_OTP_SENT`, `KILL_OTP_VERIFIED`, `KILL_OTP_FAILED`, `KILL_TOKEN_ISSUED`, `KILL_EXECUTED`, `KILL_REJECTED`, `KILL_ATTEMPT_BLOCKED` (`app/core/constants.ts`). `logcode.md`에는 기존 반영됨. |
| **설계 문서** | `docs/kill-control-phase1.md` 신규 — Kill 시퀀스, API 스펙, 로그 코드, Phase 1 체크리스트(에이전트·백엔드·OS 담당 구분). |
| **API 스텁** | `app/core/api-client.ts`에 타입(`KillProcessFingerprint`, `KillRequestResponse`, `KillVerifyOtpResponse`) 및 스텁 함수 `createKillRequest`, `verifyKillOtp`, `killExecute` 추가. 서버 미구현이므로 호출 시 throw. 백엔드 구현 후 연동 예정. |
| **다음 진행 사항** | `docs/다음_개발_진행_사항.md` §9 Kill 통제에 Phase 1 진행 상황(에이전트 완료 항목·백엔드/OS 미진행) 반영. |

**파일 변경**

- `app/core/constants.ts`: LOG_CODES에 KILL_* 8종 추가
- `docs/kill-control-phase1.md`: 신규
- `app/core/api-client.ts`: Kill 타입·스텁 함수 추가
- `docs/다음_개발_진행_사항.md`: §9 Phase 1 진행 내용 갱신
- `docs/개발_이력_리포트.md`: 33차 추가

---

## 2026-02-13 (32차) 보조 모니터 잠금화면 이슈 문서화·문서 정리

**작업 요약**

| 구분 | 내용 |
|------|------|
| **보조 모니터 이슈 문서** | `docs/보조모니터_이슈.md` 신규 작성 — 증상(배경/디폴트 색상 미적용), 시도한 대응 6종, 캡처(미러) 방식 시도·원복, 현재 상태·권장 사항 정리 |
| **문서 정리** | `docs/다음_개발_진행_사항.md`: 마지막 점검 일자 갱신, 다중 디스플레이 항목에 보조 모니터 이슈·보조모니터_이슈.md 참고 추가. `docs/다중_디스플레이_QA.md`: §4 하단에 보조 모니터 배경 이슈·보조모니터_이슈.md 참고 추가 |
| **캡처 방식** | 사용자 판단으로 캡처(미러) 방식 원복. 현재 소스는 보조도 lock.html 로드 방식 유지 |

**파일 변경**

- `docs/보조모니터_이슈.md`: 신규
- `docs/다음_개발_진행_사항.md`: 마지막 점검 일자, 다중 디스플레이 문단 수정
- `docs/다중_디스플레이_QA.md`: §4 보조 모니터 배경 이슈 참고 추가
- `docs/개발_이력_리포트.md`: 32차 추가
