...
| **추가** | Base URL(`https://api.tigris5240.com`), 공통 응답 코드(§0.4), 각 API 전체 URL 및 Request/Response 파라미터 정리 |
...
| **복구** | reportApiSuccess() 또는 retryConnectivity()로 ONLINE 복귀. 앱 재시작 시 offline-state.json 복원 |
...
- `app/core/offline-manager.ts`: **신규** — 상태 머신, 30분 유예 타이머, offline-state.json 저장/복원, reportApiFailure/reportApiSuccess/retryConnectivity
