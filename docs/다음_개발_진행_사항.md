...
의존성·검증 편의를 고려한 구현 순서입니다. 완료된 항목(설치자 레지스트리, 이석 감지·해제, 창 닫기 방지, 이석/절전 감지, 트레이 작동정보, **로그 코드 전수 반영**)은 위 **현재 완료된 항목**을 참고하세요.
...
### 2. 시업/종업 화면 표시 로직 (시업·종업 화면 설계서, FR-13)

- **목표**: 종업화면 / 시업화면 / 일반 PC 사용화면을 일관된 로직으로 결정. **옵션설정 1227**에 따른 일자변경 시각(`exCountRenewal`) 이후 시업↔종업 전환, 사용자 **출근 체크 여부 시각적 명확화**.
- **일자변경 시각 기준 (옵션 1227, API `exCountRenewal`)**: 현재 시각 < exCountRenewal → 종업화면, ≥ exCountRenewal → 시업화면
- **자율출퇴근 (옵션 1385)**: 별도 회신 필드 없음. 서버가 PC-ON·시업·종업 값을 조정해 회신하므로, 조정된 회신값으로 판단
- **핵심**: PC-ON 예외 → exCountRenewal 경계 → 종업/연장 경계 계산 → 화면 결정. 자율출근 시 옵션 1385 반영값으로 잠금화면 여부 판단
- **순서 이유**: 화면 결정의 핵심. 긴급해제·고객사 설정 등이 이 로직 위에 동작
