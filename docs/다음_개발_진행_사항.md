...
(마지막 점검: 2026-02-23, FR-15 긴급해제(비밀번호 기반) 구현 반영)
...
- **OfflineManager** (`app/core/offline-manager.ts`): 상태 ONLINE / OFFLINE_GRACE / OFFLINE_LOCKED. reportApiFailure(heartbeat 3회 또는 API 2회 연속) → OFFLINE_GRACE, 30분 유예 후 OFFLINE_LOCKED. reportApiSuccess/retryConnectivity → ONLINE
...
- **업데이트 확인 버튼**: 로그인 화면 + **작동정보 화면**(버전정보 섹션)에서 "업데이트 확인" 클릭 시 `requestUpdateCheck()` 호출, 상태별 버튼 문구·토스트 표시
...
**구현 권장 순서**: ~~**1** 로그 코드~~ ✅ → ~~**2** 시업/종업 화면~~ ✅ → ~~**3** 이석정보 서버 전송~~ ✅ → ~~**4** 오프라인~~ ✅ → **5** 긴급해제 → **6** 고객사 설정 → **7** 다중 디스플레이 → **8** 인스톨/언인스톨 → **9** Kill 통제 → **12** SmartScreen → **13** 패키징·검증. (§10·11·14~16은 위 항목들의 상세 설명·참고)
...
- **요구**: 시업/종업/이석 문구·이미지·로고, 긴급해제·이석해제 비밀번호 옵션. Draft/Publish/Rollback, Push+Polling 배포
