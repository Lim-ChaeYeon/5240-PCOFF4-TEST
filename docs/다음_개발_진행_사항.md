...
(마지막 점검: 2026-02-13, 이석정보 서버 전송 FR-12 반영)
...
- **release 워크플로우** (`.github/workflows/release.yml`): `v*` 태그 푸시 시 Windows x64·Mac 빌드 후 GitHub Release에 업로드
...
- **자동 업데이트**: `package.json`의 `publish.provider: "github"`, `repository` 설정 → 설치된 앱이 GitHub Release에서 새 버전 확인·자동 적용
