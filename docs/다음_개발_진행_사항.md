...
(마지막 점검: 2026-02-20, 자동 업데이트·버전정보·로그 폴더 보완 반영)
...
### 4. 오프라인 복구·잠금 동작 (오프라인 설계안, FR-17)

- **목표**: 오프라인 시 전용 안내 화면, 30분 유예 후 미복구 시 화면 잠금, heartbeat/이벤트 보고
- **상태**: OFFLINE_GRACE → OFFLINE_LOCKED. 감지: heartbeat 3회 연속 실패 또는 핵심 API 2회 연속 실패
- **UI**: 경과 시간(mm:ss/30:00), 다시 시도·네트워크 설정·관리자 문의. OFFLINE_LOCKED 시 PC-ON/임시연장/긴급사용 차단
- **저장**: offline-state.json. 앱 재시작 시에도 잠금 유지
- **순서 이유**: 독립 기능으로 안정성 확보. 다른 화면 로직과 병렬 가능
