...
(마지막 점검: 2026-02-20, CI·릴리스·GitHub 자동 업데이트·패키징 반영)
...
의존성·검증 편의를 고려한 구현 순서입니다. 완료된 항목(설치자 레지스트리, 이석 감지·해제, 창 닫기 방지, 이석/절전 감지, 트레이 작동정보, 로그 코드 전수 반영, **시업/종업 화면 표시 로직**)은 위 **현재 완료된 항목**을 참고하세요.
...
### 3. 이석정보 서버 전송 (이석정보 서버 전송 리포트, FR-12)

- **목표**: 이석 START/END를 세션 기반(`leaveSeatSessionId`)으로 서버 전송, 장애 내성
- **API**: `POST /reportLeaveSeatEvent.do` — eventType(LEAVE_SEAT_START/END), workSessionType, reason, occurredAt, clientVersion 등
- **재시도**: 로컬 큐(JSONL) + 지수 백오프. reason 200자 제한·마스킹
- **순서 이유**: 이석 감지·해제는 이미 완료 → 전송 레이어만 추가
