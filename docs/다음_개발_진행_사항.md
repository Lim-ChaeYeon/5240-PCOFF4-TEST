...
### ~~1. 설치자 레지스트리 (FR-09, Flow-08)~~ ✅ 완료

→ 위 "현재 완료된 항목" 참고

### ~~1. 이석 감지·해제 플로우 (Flow-02 연계, 이석·절전 감지 리포트)~~ ✅ 완료

- **구현 파일**: `app/core/leave-seat.ts`, `lock.html`, `lock.js`, `app/main/index.ts`, `app/preload/index.ts`
- **정책**: `screenType=empty` → 이석. `leaveSeatReasonYn=YES && leaveSeatReasonManYn=YES` → 사유 필수. 휴게시간(`breakStartTime~breakEndTime`) 중이면 사유 면제.
- **UI**: 이석 사유 입력 모달(`#leave-seat-modal`) — 이석 감지 시각 안내, 사유 입력, PC-ON 확인.
- **API**: `callCmmPcOnOffLogPrc(IN, reason)` — 사유 포함 또는 빈 값으로 호출.
- **로그**: `LEAVE_SEAT_DETECTED`, `LEAVE_SEAT_REASON_SUBMITTED`, `LEAVE_SEAT_BREAK_EXEMPT`, `LEAVE_SEAT_UNLOCK`
- **시뮬레이터**: Flow-02(사유필수), Flow-02b(휴게면제) 두 시나리오 PASS ✅
...
- **남은 작업**: 코드 서명(Windows Authenticode), macOS notarization, 두 OS에서 설치·실행 검증·DoD 정리
...
### ~~6. 잠금화면·에이전트 창 닫기 방지 (FR-07·FR-18 보완)~~ ✅ 완료
...
### ~~11. 이석/절전 감지 구현 (이석·절전 감지 방안 리포트)~~ ✅ 완료
