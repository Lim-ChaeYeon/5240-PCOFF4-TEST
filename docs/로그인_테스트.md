...
4. **로그인** 클릭  
   ...
   - 성공 시 `state.json`에 로그인 정보 저장 후 **메인(잠금) 화면**으로 전환
...
로그인된 상태에서 **로그인 화면으로 바로 돌아가고 싶을 때** 사용하는 숨김 단축키입니다. UI에는 노출되지 않습니다.
...
동작: `state.json`에서 로그인 정보 제거 후 로그인 화면으로 전환. 토스트에 "로그아웃됨 (개발자 핫키)" 표시.
