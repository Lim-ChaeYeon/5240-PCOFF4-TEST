...
- 로그 파일 경로: `%APPDATA%\5240 PcOff Agent\logs\` (Windows), `~/Library/Application Support/5240 PcOff Agent/logs/` (Mac).  
  `UPDATE_FOUND`, `UPDATE_FAILED` 외에 실패 원인 구분 코드(`UPDATE_SIGNATURE_INVALID`, `UPDATE_HASH_MISMATCH`, `UPDATE_NETWORK_ERROR`)로 검색하면 원인 추적에 유리합니다. 자세한 체크리스트는 [smart-screen-checklist.md](smart-screen-checklist.md) 참고.
