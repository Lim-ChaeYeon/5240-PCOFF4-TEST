...
연동·E2E를 진행하기 전에 아래가 충족되어야 합니다.

| 순서 | 항목 | 서버 측 완료 조건 |
|------|------|-------------------|
| 1 | 이석 해제 비밀번호 | `POST /verifyLeaveSeatUnlock.do`(또는 동일 역할 API) 구현 완료. 요청: ServAreaID, UserID, PcOnPass 등. 응답: code==1이면 성공. (PHP `leaveSeatUnlockVerifyUrl`만 쓸 경우 서버 신규 개발 없이 진행 가능.) |
| 2 | 잠금화면 문구 API | `getLockScreenInfo.do` 또는 `GET /api/v1/pcoff/tenants/{tenantId}/lock-policy` 구현 완료. 응답에 lockScreen.screens(before/off/leave).title·message·이미지 URL 등. (미구현 시 에이전트는 config.lockScreen·lockScreenApiUrl Fallback 사용.) |
| 3 | 임시연장 서버 반영 | `callPcOffTempDelay.do` 수신 후 **JsonMapper·프로시저·DB**에서 연장 횟수·연장 시각 갱신 로직 구현·보완 완료. 이후 `getPcOffWorkTime` 재조회 시 갱신된 pcExCount·pcOffYmdTime이 나와야 함. |
...
| 2 | 잠금화면 문구 API | 터미널 로그로 getLockScreenInfo/lock-policy 미사용 시 fallback(lockScreenApiUrl → config) 동작 확인. §2 참고. **설정 검증**: `node scripts/check-lock-screen-config.mjs` | 서버 구현 후 문구·이미지 반영 여부 확인. |
| 3 | 임시연장 서버 반영 | 임시연장 요청 후 터미널에서 `callPcOffTempDelay 응답`·`직후 getPcOffWorkTime 응답` 검색 → pcExCount·pcOffYmdTime 일치 여부. §3 참고. **로그 검증**: `node scripts/check-temp-extend-reflect.mjs [logs경로]` | 서버/DB 갱신 로직 점검 후 비교. |

**통합 실행 (서버 구현 후 권장)**  
```bash
# 시뮬레이터 run-all + 잠금화면 설정 검증 + 체크리스트 출력
node scripts/run-integration-check-1-2-3.mjs

# 위 + 임시연장 로그 반영 여부 점검(3번, logs 경로 지정)
node scripts/run-integration-check-1-2-3.mjs --logs=./logs

# 시뮬레이터 제외(설정·체크리스트만)
node scripts/run-integration-check-1-2-3.mjs --skip-simulator
```
...
- [ ] **설정 검증**: `node scripts/check-lock-screen-config.mjs` 로 config.json의 lockScreen·lockScreenApiUrl 준비 상태 확인.
...
- **스크립트**: `scripts/run-integration-check-1-2-3.mjs`(통합 연동 점검), `scripts/check-lock-screen-config.mjs`(2번 Fallback 설정 검증), `scripts/check-temp-extend-reflect.mjs`(3번 로그 검증).
